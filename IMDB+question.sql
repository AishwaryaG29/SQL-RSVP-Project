...
SELECT 
    ...
    (SELECT COUNT(movie_id) FROM genre) AS genre_count,
    (SELECT COUNT(movie_id) FROM director_mapping) AS director_mapping_count,
    (SELECT COUNT(id) FROM names) AS names_count,
    (SELECT COUNT(movie_id) FROM ratings) AS ratings_count,
    (SELECT COUNT(movie_id) FROM role_mapping) AS role_mapping_count;
...
-- Type your code below:
SELECT COUNT(*) AS number_movies_usa_india
FROM movie
...
FROM (
    ...
    FROM (
        ...
        JOIN ratings r ON m.id = r.movie_id
        GROUP BY m.production_company
    ) AS ProductionVoteCounts
) AS RankedProductionHouses
...
SELECT a.name AS actor_name,
    ...
    RANK() OVER (
        ORDER BY 
            SUM(r.avg_rating * r.total_votes) / NULLIF(SUM(r.total_votes), 0) DESC, 
...
SELECT genre,avg_duration,
    SUM(avg_duration) OVER (PARTITION BY genre ORDER BY movie_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_total_duration,
    AVG(avg_duration) OVER (PARTITION BY genre ORDER BY movie_date ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS moving_avg_duration
FROM (
    SELECT
        g.genre,
...
SELECT n.name AS actress_name, 
    ...
    RANK() OVER (ORDER BY COUNT(*) DESC, SUM(r.total_votes) DESC) AS actress_rank
FROM movie m
JOIN 
    genre g ON m.id = g.movie_id
JOIN 
...
GROUP BY n.name ORDER BY actress_rank
LIMIT 3;
...
SELECT 
    ...
    AVG(DATEDIFF(m.date_published, prev_dates.prev_date)) AS avg_inter_movie_days,
...
LEFT JOIN
    (SELECT
        ...
        m.id AS movie_id,
        m.date_published,
        LAG(m.date_published) OVER (PARTITION BY dm.name_id ORDER BY m.date_published) AS prev_date
    FROM director_mapping dm
    JOIN
        movie m ON dm.movie_id = m.id) AS prev_dates
ON
    dm.name_id = prev_dates.name_id AND m.id = prev_dates.movie_id
